...
class TextProcessor(Processor):
    ...
    def _chunk_elements(payload: ProcessorPayload):
        ...
        has_title = any(
            type(element).__name__ == "Title" for element in payload.elements
        )
    ...
    async def _process_image_chunk(
        ...
        return await self._image_processor.process(processor_payload)
    ...
    async def process(self, payload):
        ...
        # Chunk
        raw_chunks = await asyncio.to_thread(self._chunk_elements, payload)
        ...
        for chunk in document_chunks:
            ...
            has_table, has_image = self._embedded_elements(chunk)
            ...
            # TODO:
            # We shouldn't blindly feed a table to its processor because it could
            # be split into different chunks.
            # If we want better chunking for tables in PDFs, we must collect all
            # tables in one piece.
            ...
            # if has_table:
            #     table_chunks = await self._table_processor.process(
            #         replace(
            #             payload,
            ...
            #         )
            #     )
            #     embedded_chunks.extend(table_chunks)
